...
class GaussianLDACholTrainer_TextAudio_token_bigram:
    def __init__(self, corpus,audio_corpus, audio_features,vocab_embeddings, vocab,num_tables, multimodal = True,alpha=None, kappa=0.1, log=None, save_path=None,
    ...
    def log_multivariate_tdensity_chol_audio(self, x, table_id):
        """
        Gaussian likelihood for a table-embedding pair when using Cholesky decomposition.
        ...
        if x.ndim > 1:
            logprobs = np.zeros(x.shape[0], dtype=np.float64)
            for i in range(x.shape[0]):
                logprobs[i] = self.log_multivariate_tdensity_chol_audio(x[i], table_id)
            return logprobs
        ...
        # therefore I am gonna use triangular solver
        # first calculate (x-mu)
        x_minus_mu = x - self.table_means_audio[table_id]
        # Now scale the lower tringular matrix
        ltriangular_chol = scaleTdistrn * self.table_cholesky_ltriangular_mat_audio[table_id]
        solved = solve_triangular(ltriangular_chol, x_minus_mu)
        # Now take xTx (dot product)
        val = (solved ** 2.).sum(-1)
    ...
    def sample(self, num_iterations):
        ...
        log_prev_word_token = 0
        log_prev_audio_token = 0
        for iteration in range(num_iterations):
            ...
            for d, doc in enumerate(pbar(self.corpus)):
                ...
                for w, cust_id in enumerate(doc):
                    ...
                    log_posteriors_text = np.log(counts_text) + log_lls_text + log_prev_word_token
                    
                    if w == 0:
                        log_prev_word_token = log_lls_text
                    else :
                        log_posteriors_text = log_posteriors_text + log_prev_word_token
                        log_prev_word_token = log_lls_text
                    ...
                    for frame_index in range(frame_start,pad):
                        ...
                        if frame_index == 0:
                            ...
                            log_prev_audio_token = log_posteriors_audio + log_prev_audio_token
                            log_prev_word_token = log_lls_text
