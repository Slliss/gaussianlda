...
class GaussianLDATrainer_chol_audio_sequence:
    def __init__(self,audio_corpus, audio_features, num_tables, alpha, log=None, save_path=None,
        ...
        # dataVectors
        self.vocab_embeddings = audio_features
        self.embedding_size = audio_features.shape[1]
        # List of list of ints
        self.corpus = audio_corpus
        # numIterations
        # K, num tables
        self.num_tables = num_tables
        # N, num docs
        self.num_documents = len(audio_corpus)
    ...
    def update_table_params_chol(self, table_id, cust_id, is_removed=False):
        ...
        if is_removed:
            ...
            x = (cust_id - self.table_means[table_id]) * np.sqrt((k_n + 1.) / k_n)
            ...
            x = (cust_id - self.table_means[table_id]) * np.sqrt(k_n / (k_n - 1.))
            ...
            for d, doc in enumerate(pbar(self.corpus)):
                ...
                for w, cust_id in enumerate(doc):
                    ...
                    if w == 0:
                        ...
                        log_prev_token_prob = self.log_multivariate_tdensity_tables(doc[w-1])
                    
                    # Add log prior in the posterior vector
                    log_posteriors = np.log(counts) + log_lls + log_prev_token_prob
