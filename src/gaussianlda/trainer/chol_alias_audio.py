...
class GaussianLDAAliasTrainer_Audio:
    def __init__(self, audio_corpus, audio_features, num_tables, alpha=None, kappa=0.1, log=None, save_path=None,
        ...
        self.num_terms = audio_features.shape[0]
    ...
    def update_table_params(self, table_id, cust_id, is_removed=False):
        ...
        if is_removed:
            ...
            x = (cust_id - self.table_means.np[table_id]) * np.sqrt((k_n + 1.) / k_n)
            ...
            x = (cust_id - self.table_means.np[table_id]) * np.sqrt(k_n / (k_n - 1.))
